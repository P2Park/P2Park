...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("blockreward",    (uint64_t)GetProofOfWorkReward(0)));
    ...
    obj.push_back(Pair("stakereward",    (uint64_t)GetProofOfStakeReward(0,0,0)));
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "P2Park is not connected!");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "P2Park is not connected!");
